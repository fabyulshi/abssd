...
public class user extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ps1 = con.prepareStatement("select * from user where email='"+ email + "' AND mobno='" + pwd + "' ");
			ResultSet rs = ps1.executeQuery();
			String fname="",lname="";
			if (rs.next()) {
				int flag = send.EmailSending(email, "Your New Password", "New Password: "+pass);
				...
				if(flag>0)
				{
					...
					ps.executeUpdate();
					response.sendRedirect("user.jsp?done");
				}
				else
				{
					response.sendRedirect("user.jsp?fail");
				}
			}
			else
			{
				response.sendRedirect("forgot.jsp?login");
			}
